...
import 'package:nitro_fold_duo/nitro_fold_duo.dart' as plugin;
...
class _DemoPageState extends State<_DemoPage> {
  ...
  Future<void> _runAdd() async {
    ...
    try {
      final v = plugin.NitroFoldDuo.instance.add(3, 4);
  ...
  Future<void> _runGreeting() async {
    setState(() { _loading = true; _result = '—'; });
    try {
      final s = await plugin.NitroFoldDuo.instance.getGreeting('World');
      setState(() => _result = s);
    } catch (e) {
      setState(() => _result = 'Error: $e');
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('NitroFoldDuo Demo')),
      body: Center(
        child: Column(
